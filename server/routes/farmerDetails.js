...
router.get('/All', async (req, res) => {
  try {
    const [results] = await db.execute('SELECT * FROM FarmerDetails');
    res.json(results);
  } catch (err) {
...
router.get('/oneFarmer/:id', async (req, res) => {
  ...
  try {
    const [results] = await db.execute('SELECT * FROM FarmerDetails WHERE ID = ?', [farmerId]);
    if (results.length === 0) {
...
router.post('/Add', async (req, res) => {
  ...
  try {
    await db.execute('INSERT INTO FarmerDetails (Name, FarmName, Telephone, Email, Address, QuantityOfDunams, WorkingDays, BriefExplanationOfWork, IDNumber, RelevantStartTime, RelevantEndTime, DesiredVolunteersCount, ACTIV) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', 
